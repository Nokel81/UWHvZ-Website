    ...
    return new Promise(function(resolve, reject) {
        ...
        Game.findOne({
            ...
            .then(signups => {
                ...
                return User.find({
                    email: {
                        $in: signups.map(signup => signup.userEmail)
                    }
                }).select("_id email").exec();
            })
            .then(users => {
                game.zombies = game.originalZombies = users.filter(user => OZemails.indexOf(user.email) >= 0).map(user => user._id);
            ...
            .then(users => {
                return Promise.join(sendStartingEmail(spectatorEmails, "Spectator", game, HTMLlore, fileData),
                    ...
                    sendStartingEmail(OZemails, "Zombie", game, HTMLlore, fileData, clone(users).map(user => user.playerName)));
            })
            .then(() => {
